...
class Zmanim
  ...
  hadlakatNerot: -> @_hadlakatNerot ?= (
    ...
    if hebrewDate.isShabbat()
      ...
      "After #{moment.max(@setHaKochabim3Stars().seconds(60), moment(@sunset()).seconds(0).add('minutes', 46)).format('h:mm')}#{if showEarlyTime then "<small><br>(After #{moment(@plag()).seconds(60).format('h:mm')} / eat from all<br>cooked foods before #{moment(@sunset()).seconds(0).format('h:mm')})</small>" else ""}"
